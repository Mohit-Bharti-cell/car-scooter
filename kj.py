...
def create_table(cursor):
    """Create the cars table with unique segment_id and segment_name constraint."""
    cursor.execute("""
...
def insert_car_details(car_name, segment_id, segment_name, model_type, year, engine_type, fuel_type, price, image_urls, cursor):
    # Check if the car already exists under the same segment_id and model_type (to prevent duplicate cars)
    cursor.execute("""
        SELECT 1 
        FROM cars 
